...
public class Ventas 
{
	 ...
	 // metodo a construir
		...
		try 
		{
			...
			while(linea!=null)
			{
				...
				this.productos[i].setValorProducto(Double.parseDouble(pr[2]));
				linea = br.readLine();
				i++;
			}
		} 
	...
	public void consolidarVentas(double IVA) {
		...
		{
			cant=detalleVentas[i].getCantidad();
			PProducto=detalleVentas[i].getValorUnitario();
			VT=VT+(cant*PProducto);
		}
		valorIVA=VT*(IVA/100);
		valorTotal=VT+IVA;
		this.valormasIVA=VT;
	...
	public String generarReporteVentas() {
		...
		reporte+=" ** Total Ventas: "+valorTotal+"\n";
		reporte+=" ** Valor de IVA: "+valorIVA+" \n";
		reporte+=" ** Total con IVA: "+valormasIVA+ "\n";
